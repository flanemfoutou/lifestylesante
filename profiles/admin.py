...
class BaseAdmin(admin.ModelAdmin):
    ...
    list_filter = ('isDeleted',)  # Ajout du filtre pour voir les objets supprimés
    
    def get_queryset(self, request):
        """
        Filtre par défaut pour exclure les objets supprimés, sauf si le filtre isDeleted=True est utilisé.
        """
        qs = super().get_queryset(request)
        if not request.GET.get('isDeleted__exact') == 'True':
...
class EmployeAdmin(BaseAdmin):
    ...
    list_filter = ('sexe', 'fonction', 'departement', 'isDeleted')
...
class MarquerArriveeAdmin(BaseAdmin):
    ...
    list_filter = ('arrivee', 'date_arrivee', 'isDeleted')
...
class MarquerDepartAdmin(BaseAdmin):
    ...
    list_filter = ('depart', 'date_depart', 'isDeleted')
